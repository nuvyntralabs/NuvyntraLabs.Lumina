...
public partial class SignInViewModel : PageViewModel
{
    ...
    private async Task SignInAsync(CancellationToken cancellationToken)
    {
        ...
        await Navigator!.ResetAsync<HomeViewModel>(cancellationToken).ConfigureAwait(false);
    }
    [AsyncModelCommand]
    private Task OpenHomeAsync(CancellationToken cancellationToken)
        => Navigator!.ResetAsync<HomeViewModel>(cancellationToken);
    [AsyncModelCommand]
    private Task OpenSignUpAsync(CancellationToken cancellationToken)
        => Navigator!.NavigateToAsync<SignUpViewModel>(cancellationToken);
    [AsyncModelCommand]
    private Task OpenForgotPasswordAsync(CancellationToken cancellationToken)
        => Navigator!.NavigateToAsync<ForgotPasswordViewModel>(cancellationToken);
